...
2.5++ MAR 2019	Added option for RCA M records. HRJ
      Jun 2019  added DS as "BLK" HRJ

2.6   Jun 2020  $hex %bin @oct as numeric prefixes not for lables - HRJ
                for binary output, fill is 0FFH - HRJ
2.6+  June 2021  HRJ add .1 .0 notation for hi lo byte
	  Jan 2022 HRJ fixed DB, immediate evaluation for NOT (V flagged values) ff80 wrong ff00 right
...
char hformat='I'; /* (I)ntel hex or RCA (M) format*/
int pass = 0;
int eject, filesp, forwd, listhex, caphex = 1;
...
char **argv;
{
	...
	printf("fixes for LCC/Windows by HRJ update Jan 2022\n");
    printf("A18 source_file | -l list_file | -o/-m/-b object_file \n\n");
    ...
    fclose(filestk[0]);  lclose();  hclose(); bclose();
...
static void pseudo_op(void)
{
	...
	case FILL:  do_label();   /* fill value v, count of bytes u */
		    if ((v = expr()) > 255) { v = 0;  error('V'); }  /* byte value only */
	 		if ((u = expr()) > MAXLINE) { u = 0;  error('V'); } /* array is limited */
			for (i=0; i<u; obj[i++] = v);
			bytes = u;
		    break;
