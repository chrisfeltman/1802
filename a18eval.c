	...
	switch (token.attr & TYPE) {
	    ...
	    case STR:	for (;;) {
				...
				case OPR:   if (!(token.attr & BINARY)) {
					    ...
					    if ((token.attr & PREC) >= pre) { /* check precedence */
						unlex();  return u;
					    }
					    if (op != ')')
						...
						case '.':  if (v == 0) u = u & 0xFF; /*HRJ hi low dot*/
                 				else if (v == 1) u = u >> 8;
                 				else exp_error('E');
                 				break;
...
TOKEN *lex(void) /* function lex() */
{
	...
	if (o = find_operator(token.sval)) {
		...
		if (token.sval[0] == '$') { /* HRJ $nnnn */
			...
			/* printf("H$ "); hrj */
			token.sval[0] = '0';  b = 8; make_number(b);
		}
	...
	*p = '\0';  make_number(b);
    ...
    case '.':   token.attr = BINARY + ADDIT + OPR; /*HRJ hi lo dot*/
               goto opr1;
...
int isalph(char c) /* remove @ ? to allow octal prefix HRJ */
{
    return (c >= '@' && c <= '~') || c == '!' || c == '#' || c == '$' || c == '%' || c == '&' || c == ':' || c == '?';
	/* HRJ remove '.' for hi lo dot */
}
