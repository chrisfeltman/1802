...
2.5	AUG 1988	Fixed a bug in the command line parser that puts it
	...
	June 2018	Added / slash representations L. Christensen
     Mar 2019	Added M-record format HRJ.
	Jun 2019	Added "DS" as "BLK" (space no allocation) HRJ.
	Jun 2020	bseek() routine now fills with 0xFF HRJ
...
extern char hformat; /* (I)ntel hex or  RCA (M) format*/
extern int eject, listhex, caphex;
...
/*  Pass 1: Add new symbol to symbol table.  Returns pointer to symbol even if	*/
...
char *nam;
{
	...
	/* printf("new_symbol>>%s<<\n",nam);  HRJ diagnostic*/
    for (p = &sroot; (q = *p) && (i = strcmp(nam,q -> sname));  /*  HRJ moved   right par? */
...
/*  Pass 2: Look up symbol in symbol table.  Returns pointer to symbol or NULL	*/
/*  if symbol not found.						*/
...
void lputs()
{
	...
	do {
	    ...
	    if (listhex) {
		fprintf(list,(caphex ? "%04X  " : "%04x  "), address);
		for (j = 4; j; --j) {
		    if (i) {
				--i;  ++address;
...
/*  fill the range with 0xFF...unless it's the start where the binaddress is 0. */
...
unsigned a;
{
	...
	if (bin) {
	  ...
	  if ((binaddr == 0) && (a != 0)){
		  ...
		  printf("binary file address change from %4X to %4X, fill with 0FFH!\n",binaddr, a);
          for (i = binaddr; i < a; bputc(0xff), i++);
	  }
...
char *nam;
{
    ...
    if (hex) {
		warning(TWOHEX); return;
	}
    if (!(hex = fopen(nam,"w"))) fatal_error(HEXOPEN);
	if (hformat=='M') { /* HRJ RCA M records */
        putc ('!',hex); putc ('M',hex); putc ('\n',hex);
	}
    return;
}
...
static void mrecord(typ)
unsigned typ;
/* produces RCA M-type data format data for type 0,
   comment ";adr" for address record for type 1. */
{
    SCRATCH unsigned i;
    ...
    if (typ==0) { /* data record */
       ...
       for (i = 0; i < cnt; ++i) {
         putc (' ',hex);
	    putb(buf[i]); /* space and data byte*/
       }
        putc (';',hex); putc ('\n',hex);
		addr += cnt;  cnt = 0;
       sum = 0; /* HRJ */
    ...
    if (typ==1) { /* addr record as comment */
       putc(';',hex); putc ('a',hex);
       putc('d',hex); putc ('r',hex); putc (' ',hex);
       putb(high(addr)); putb(low(addr)); putc ('H',hex);
       putc('\n',hex);
		addr += cnt;  cnt = 0;
       sum = 0; /* HRJ */    }
