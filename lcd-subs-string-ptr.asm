...
resetmsg:	
	load R8, string_array

next_string:
	lda R8				; load the next 16-bit address into RA via R8
	phi RA
	lda R8
	plo RA
	
	bnz msg_out			; if the low byte was not 00 we are not at end
	ghi RA				; look at the high byte
	bnz msg_out			; if also not 00, we are not at end
	br resetmsg			; we have reached the end so reset
	
msg_out:	
	sep RB				; call output 
	br next_string 		; back for next string
...
mystring7 TEXT "On earth, as it is in cyberland"
	db 0;

mystring8 TEXT "Be careful! I byte!!!"
	db 0;

mystring9 TEXT "Just kidding! I only nybble... ;-)"
	db 0;

mystring10 TEXT "Would you like to play a game???"
	db 0;

string_array 		; array of pointers to strings
	dw mystring
	dw mystring2
	dw mystring3
	dw mystring4
